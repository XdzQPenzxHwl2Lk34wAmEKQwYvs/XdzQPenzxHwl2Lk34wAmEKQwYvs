...
local Blacklisted_GameIds = loadstring(game:HttpGet("https://raw.githubusercontent.com/Definezzz/melancholia/main/blacklisted_games"))()
...
getgenv().notify = function(txt) 
    if VisualSense.Options["Allow Notifications"] then
        if string.lower(VisualSense.Options["Notifcation Type"]) == "akali" then 
            gggg({
                Description = txt;
                Title = "Notification";
                Duration = VisualSense.Options["Notifcation Duration"];
            });
        elseif string.lower(VisualSense.Options["Notifcation Type"]) == "roblox" then
            game:GetService("StarterGui"):SetCore("SendNotification",{
                Title = "Notification",
                Text = txt, 
                Icon = "rbxassetid://18129564380",
        ...
        elseif string.lower(VisualSense.Options["Notifcation Type"]) == "xaxa" then
...
local CFrame = VisualSense.PlayerSettings.CFrame.Enabled
    repeat
        if VisualSense.PlayerSettings.CFrame.Enabled then
            LocalPlayer.Character.HumanoidRootPart.CFrame = LocalPlayer.Character.HumanoidRootPart.CFrame + LocalPlayer.Character.Humanoid.MoveDirection * VisualSense.PlayerSettings.CFrame.Speed / 4
        else
...
if VisualSense["Options"]["Internal"].Enabled then 
    if VisualSense_UI_LOADED then 
        ...
        AntiLockTab:AddDropdown('AntiLocks/Method', {
            Values = { 'Custom', 'Prediction Changer' ,'Prediction Disabler', 'Up', 'Down','Prediction Tripler','Prediction Reverser','LookVector','AirOrthodox','Prediction Multiplier', 'Spinbot Desync'},
        ...
        PlayerSettings:AddToggle('CFrame', {
        ...
        PlayerSettings:AddSlider('PlayerSettings/CFrame/Speed', {
            Text = 'Speed',
            Default = VisualSense["PlayerSettings"].CFrame.Speed,
            Min = 0,
            Max = 0.10,
            Rounding = 3,
            Compact = true,
